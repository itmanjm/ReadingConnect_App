...
def add_entry(
    ...
    # Check for duplicate
    cursor.execute(
        "SELECT id, content FROM memory_entries WHERE content_hash = ?", (content_hash,)
    )
...
def list_entries(
    ...
    # Get total count
    cursor.execute(
        f"SELECT COUNT(*) as count FROM memory_entries WHERE {where_clause}", params
    )
...
def search_entries(
    query: str, entry_type: Optional[str] = None, limit: int = 20
) -> Dict[str, Any]:
    """
...
def update_entry(entry_id: int, **kwargs) -> Dict[str, Any]:
    ...
    cursor.execute(
        f"UPDATE memory_entries SET {', '.join(updates)} WHERE id = ?", values
    )
    conn.commit()
    ...
    return {
        "success": True,
        "entry": entry,
        "message": f"Memory entry {entry_id} updated",
    }
...
def add_daily_log(
    date: str, summary: str, raw_log: str, key_events: Optional[List[str]] = None
) -> Dict[str, Any]:
    """
...
def main():
    ...
    parser.add_argument(
        "--type",
        help="Memory type (fact, preference, event, insight, task, relationship)",
    )
    parser.add_argument(
        "--source",
        default="session",
        help="Source (user, inferred, session, external, system)",
    )
    parser.add_argument(
        "--confidence", type=float, default=1.0, help="Confidence score 0-1"
    )
    parser.add_argument(
        "--importance", type=int, default=5, help="Importance level 1-10"
    )
    parser.add_argument("--tags", help="Comma-separated tags")
    parser.add_argument("--context", help="Context about when/why this was learned")
    parser.add_argument("--query", help="Search query")
    parser.add_argument(
        "--hours", type=int, default=24, help="Hours for recent entries"
    )
