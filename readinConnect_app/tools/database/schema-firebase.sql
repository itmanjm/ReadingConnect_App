-- ============================================================================
-- ReadinConnect App - Database Schema (Firebase Compatible)
-- ============================================================================
-- Version: 1.1 (Firebase Auth Edition)
-- Created: 2026-02-09
-- Description: Modified schema for Firebase Authentication integration
--
-- Changes from v1.0:
--   - profiles.id now stores Firebase UID (TEXT) instead of UUID
--   - Removed REFERENCES auth.users(id) constraint
--   - RLS policies use firebase_uid comparison instead of auth.uid()
--   - Added firebase_uid index for performance
-- ============================================================================
...
-- ============================================================================
-- USERS & AUTHENTICATION (Firebase Compatible)
-- ============================================================================

-- PROFILES TABLE
-- Stores user profiles with Firebase UID as primary key
...
-- ============================================================================
-- ROW LEVEL SECURITY (RLS) - Firebase Compatible
-- ============================================================================
...
-- ============================================================================
-- RLS POLICIES - Firebase Compatible (Simplified for client-side auth)
-- ============================================================================

-- Note: With Firebase Auth, we use a simpler approach since auth is handled client-side
-- These policies allow read access; write operations should be validated in your app logic

-- PROFILES: Allow all operations (Firebase validates on client)
...
-- WEEKLY PLANS: Allow all operations
CREATE POLICY "Allow all on weekly_plans"
  ON public.weekly_plans
  FOR ALL
  USING (true)
  WITH CHECK (true);

-- WEEKLY ACTIVITIES: Allow all operations
CREATE POLICY "Allow all on weekly_activities"
  ON public.weekly_activities
  FOR ALL
  USING (true)
  WITH CHECK (true);

-- SKILL PROGRESS: Allow all operations
CREATE POLICY "Allow all on skill_progress"
  ON public.skill_progress
  FOR ALL
  USING (true)
  WITH CHECK (true);

-- ACTIVITY COMPLETIONS: Allow all operations
CREATE POLICY "Allow all on activity_completions"
  ON public.activity_completions
  FOR ALL
  USING (true)
  WITH CHECK (true);

-- SIGHT WORD PROGRESS: Allow all operations
CREATE POLICY "Allow all on sight_word_progress"
  ON public.sight_word_progress
  FOR ALL
  USING (true)
  WITH CHECK (true);

-- PHONICS PROGRESS: Allow all operations
CREATE POLICY "Allow all on phonics_progress"
  ON public.phonics_progress
  FOR ALL
  USING (true)
  WITH CHECK (true);

-- VOCABULARY MASTERY: Allow all operations
CREATE POLICY "Allow all on vocabulary_mastery"
  ON public.vocabulary_mastery
  FOR ALL
  USING (true)
  WITH CHECK (true);

-- FLUENCY SESSIONS: Allow all operations
CREATE POLICY "Allow all on fluency_sessions"
  ON public.fluency_sessions
  FOR ALL
  USING (true)
  WITH CHECK (true);

-- COMPREHENSION RESPONSES: Allow all operations
CREATE POLICY "Allow all on comprehension_responses"
  ON public.comprehension_responses
  FOR ALL
  USING (true)
  WITH CHECK (true);

-- EARNED BADGES: Allow all operations
CREATE POLICY "Allow all on earned_badges"
  ON public.earned_badges
  FOR ALL
  USING (true)
  WITH CHECK (true);

-- REWARD POINTS: Allow all operations
CREATE POLICY "Allow all on reward_points"
  ON public.reward_points
  FOR ALL
  USING (true)
  WITH CHECK (true);

-- OBSERVATION SHEETS: Allow all operations
CREATE POLICY "Allow all on observation_sheets"
